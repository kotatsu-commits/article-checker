"""
記事チェッカー
名簿・参考資料と照合して記事内の固有名詞の誤りを検出し、
AIによる校閲（語彙・事実確認・時制整合性など）も行います。
"""
...
# 連数字（新聞組版の外字）の正規化
# ════════════════════════════════════════════════
# 新聞組版ソフトでは、縦組み時に2桁の数字を1文字分に収める「連数字」という
# 外字（PUA: 私用領域）を使う。"00"→U+F1CF から "99"→U+F232 まで、
# コードポイントは数値と完全に連続しているため、テーブル引き不要で変換できる。
RENJISUUJI_BASE = 0xF1CF  # "00" に対応する先頭コードポイント


def normalize_renjisuuji(text: str) -> str:
    """連数字（外字）を通常の半角数字2桁に変換する。対象外の文字はそのまま返す。"""
    return "".join(
        f"{ord(ch) - RENJISUUJI_BASE:02d}"
        if RENJISUUJI_BASE <= ord(ch) <= RENJISUUJI_BASE + 99
        else ch
        for ch in text
    )
...
def build_kouetsu_user_message(
    article: str, has_reference_docs: bool = False, use_web_search: bool = True
) -> str:
    ...
    if not use_web_search:
        basis = "記事本文の記述と、上記の添付資料" if has_reference_docs else "記事本文の記述"
        text += (
            "\n\n## インターネット検索について\n"
            "今回はインターネット検索を利用できません。事実確認は"
            f"{basis}のみを根拠に行ってください。"
            "検索でなければ確認できない項目（固有名詞の表記の完全一致確認、最新の公表情報との整合性確認等）は、"
            "検索できなかった旨を明記したうえで【要確認】として扱ってください。"
            "検索結果を推測や既存知識で代用しないでください。"
        )
    return text


def run_kouetsu(
    article: str, api_key: str, reference_docs: list = None, use_web_search: bool = True
) -> dict:
    client = anthropic.Anthropic(api_key=api_key)
    request_kwargs = dict(
    ...
    if use_web_search:
        request_kwargs["tools"] = [{"type": "web_search_20260209", "name": "web_search"}]

    user_text = build_kouetsu_user_message(
        article, has_reference_docs=bool(reference_docs), use_web_search=use_web_search
    )
    if reference_docs:
...
def main():
    st.title("📰 記事チェッカー")
    st.caption(
        "AIによる校閲（語彙・助詞・事実確認・時制整合性など）と、"
        "名簿・参考資料に基づく固有名詞照合の2つの機能で記事の誤りを検出します。"
    )
    ...
    with st.sidebar:
        ...
        st.divider()
        st.header("📥 データの読み込み")
        tab_reference, tab_roster = st.tabs(["📎 参考資料", "📋 名簿"])
    ...
    # 組版ソフトから貼り付けた際に連数字（外字）が残っていても、AIが数値として認識できるようにする
    article = normalize_renjisuuji(article)

    tab_kouetsu, tab_check = st.tabs(["📝 AI校閲", "✅ 固有名詞チェック"])
    ...
    with tab_kouetsu:
        st.caption(
            "語彙・助詞、事実確認（固有名詞・数値・時制・主体関係等）、表記の観点でAIが校閲します。"
        )
        use_web_search = st.checkbox(
            "🔍 インターネット検索を使用する",
            value=True,
            key="kouetsu_use_web_search",
            help=(
                "オフにすると検索は行わず、記事本文と添付した参考資料のみを根拠に校閲します。"
                "検索でしか確認できない項目は「要確認」として扱われます。"
            ),
        )
        ...
        if run_kouetsu_btn:
            ...
            spinner_text = (
                "AIが校閲中です…（インターネット検索を伴うため1分程度かかることがあります）"
                if use_web_search
                else "AIが校閲中です…"
            )
            with st.spinner(spinner_text):
                try:
                    st.session_state["kouetsu_result"] = run_kouetsu(
                        article.strip(), api_key, reference_docs, use_web_search
                    )
                except anthropic.AuthenticationError:
                    st.error("APIキーが無効です。正しいキーを入力してください。")
                except Exception as exc:
