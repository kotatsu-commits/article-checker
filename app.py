...
def build_html(article: str, entities: list) -> str:
    ...
    cursor = 0
    for ann in merged:
        parts.append(html.escape(article[cursor:ann["start"]]))
        ent = ann["ent"]
        ...
        cursor = ann["end"]
    parts.append(html.escape(article[cursor:]))
    parts.append("</div>")
    return "".join(parts)
...
def main():
    ...
    if "result" in st.session_state:
        ...
        st.markdown(build_html(saved_article, active), unsafe_allow_html=True)
